...
public class RevivalChance implements EventSpace
{
	...
	private void playRevivalChance()
	{
		...
		int delay = 1000 + rng.nextInt(500);
		int revivalTarget = -1;
		...
		while(delay < 2500)
		{
			...
			revivalTarget = rng.nextInt(candidates.size()+1) - 1;
			chosenPrize = RevivalPrize.values()[r.nextInt(RevivalPrize.values().length)];
			if(revivalTarget == -1)
				targetName = "no one";
			else
	...
	void failedRevival()
	{
		failedRevival(RevivalPrize.values()[r.nextInt(RevivalPrize.values().length)]);
	}
	...
	void newBomb(Player target)
	{
		if(target.isBot)
		{
			...
			if(!safeSpaces.isEmpty())
				 bombPosition = safeSpaces.get(rng.nextInt(safeSpaces.size()));
