...
public class Bowser implements EventSpace
{
	...
	public void execute(GameController game, int player)
	{
		...
		else
		{
			...
			game.channel.sendMessage(String.format(INTRO_MESSAGES[r.nextInt(INTRO_MESSAGES.length)],
		...
		{
			game.channel.sendMessage(EVENT_MESSAGES[r.nextInt(EVENT_MESSAGES.length)]).queue();
		}
		try { Thread.sleep(1000); } catch (InterruptedException e) { e.printStackTrace(); }
		game.channel.sendMessage(ROULETTE_MESSAGES[r.nextInt(ROULETTE_MESSAGES.length)]).queue();
