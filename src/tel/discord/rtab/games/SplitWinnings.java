...
public class SplitWinnings extends MiniGameWrapper {
	...
	static final List<Integer> BASE_CASH = Arrays.asList(10_000, 10_000, 15_000, 15_000, 20_000, 20_000, 25_000, 25_000,
			30_000, 35_000, 40_000, 45_000, 50_000, 0, 0, 0);
	static final List<Double> BASE_MULTIPLIERS = Arrays.asList(1.5, 1.5, 1.5, 1.5, 2.0, 2.0, 2.0, 2.5, 2.5,
    ...
    String getBotPick()
    {
        ...
        else
        {
    		...
    		for(int i=BOARD_SIZE; i<2*BOARD_SIZE; i++)
    			if(!pickedSpaces[i])
    				openSpaces.add(i+1);
        	//Trial run if we're eligible to stop
        	if((scores[stage] >= getMaxBank() || enhanced)
        			&& multipliers.get(openSpaces.get((int)((RtaBMath.random()*openSpaces.size())-1)) - BOARD_SIZE) <= 0.0)
    			return "STOP";
        	else
        		return String.valueOf(openSpaces.get((int)(RtaBMath.random()*openSpaces.size())));
        }
